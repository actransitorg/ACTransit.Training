...
namespace ACTransit.Training.Web.Infrastructure
{
    public class CustomErrorAttribute : HandleErrorAttribute
    {
        ...
        {         
            ...
            if (filterContext.HttpContext.Response.StatusCode != (int)HttpStatusCode.NotFound && !(currentException is FriendlyException))
                Email.EmailError(currentException);
            ...
            else
            {
                ...
                if (exception != null)
                {
                    if (((ViewResult) filterContext.Result).TempData.ContainsKey("ErrorMessage"))
                        ((ViewResult) filterContext.Result).TempData["ErrorMessage"] = exception.Message;
                    else
                        ((ViewResult) filterContext.Result).TempData.Add("ErrorMessage", exception.Message);
                }
